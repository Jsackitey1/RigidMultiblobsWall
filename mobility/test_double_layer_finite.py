...
if __name__ == '__main__':
  ...
  if wall:
    ...
    vn = -np.einsum('bi,bi->b', vector_src, normals_src) 
    velocity_pressure = np.zeros((N_src, 3))
    x = np.zeros((N_src, 3))
    x[:,0] = 1
    ...
    velocity_finite = (velocity_finite_np - velocity_finite_nm + velocity_finite_vp - velocity_finite_vm) * (eta / dx) + velocity_pressure 
